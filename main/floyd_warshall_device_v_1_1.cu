...
//rounds code
__global__ void execute_round_device_v_1_1_phase_1(int *matrix, int n, int t, int B);
__global__ void execute_round_device_v_1_1_phase_2(int *matrix, int n, int t, int B);
__global__ void execute_round_device_v_1_1_phase_3(int *matrix, int n, int t, int B);
...
void floyd_warshall_blocked_device_v_1_1(int *matrix, int n, int B) {
    ...
    for(int t = 0; t < num_rounds; t++) { 
        ...
        execute_round_device_v_1_1_phase_1<<<num_blocks_phase_1, threads_per_block_phase_1>>>(dev_rand_matrix, n, t, B);
        ...
        execute_round_device_v_1_1_phase_2<<<num_blocks_phase_2_3, threads_per_block_phase_1>>>(dev_rand_matrix, n, t, B);
        HANDLE_ERROR(cudaDeviceSynchronize());

        // phase 3: all the remaining blocks, so all the blocks that don't share a row or a col with t

        execute_round_device_v_1_1_phase_3<<<num_blocks_phase_2_3, threads_per_block_phase_1>>>(dev_rand_matrix, n, t, B);
        HANDLE_ERROR(cudaDeviceSynchronize()); 
    }
